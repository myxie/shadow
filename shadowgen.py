...
if __name__ == '__main__':
    ...
    daliuge_parser = subparsers.add_parser('daliuge',
                                           help='Unroll and Translate DALiuGE Logical Graph')
    # This is what we get get when we do func(vars(args)...
    daliuge_parser.add_argument('lg',
                                help='The logical graph that needs translating')
    daliuge_parser.add_argument('--nc', help='Edit the number of channels')
    daliuge_parser.set_defaults(func=run_daliuge_translator)

    ggen_parser = subparsers.add_parser('ggen',
                                        help=' Generate sample dataflow graphs using ggen')
    ggen_parser.set_defaults(func=ggen_generation)

    dax_parser = subparsers.add_parser('dax',
                                       help='Translate DAX files to shadow format')
