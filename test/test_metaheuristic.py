...
class TestPopulationGeneration(unittest.TestCase):
    ...
    def setUp(self):
        self.wf = Workflow("{0}/{1}".format(current_dir,
                                            cfg.test_metaheuristic_data[
                                                'topcuoglu_graph']))
        env = Environment("{0}/{1}".format(current_dir,
                                           cfg.test_metaheuristic_data[
                                               'graph_sys_with_costs']))
    ...
    def test_generate_exec_orders(self):
        ...
        # Test 1
        top_sort = generate_exec_orders(self.wf, popsize=4, rng=self.rng,
                                        skip_limit=1)
    ...
    def test_calc_start_finish_time(self):
        ...
        machine = self.wf.env.machines[0]

        solution = GASolution(self.wf.env.machines)
        ast, aft = calc_start_finish_times(
            curr[0], machine, self.wf, solution
        )
        self.assertEqual(0, ast)
        self.assertEqual(17, aft)
    ...
    def test_generate_allocations(self):
        ...
        self.assertEqual(soln.makespan, 145)
    ...
    def test_pop_gen(self):
        ...
        self.assertEqual(145, soln1.makespan)
        soln2 = pop[2]
        self.assertNotEqual(114, soln2.makespan)
        soln5 = pop[4]
        self.assertEqual(159, soln5.makespan)
        soln25 = pop[24]
        self.assertEqual(165, soln25.makespan)
    ...
    def test_create_sample_pop(self):
        ...
        for soln in pop:
            ...
            logger.debug(
                "Makespan (s): {0}".format(calculate_fitness(['time'], soln)))
            logger.debug(
                "Cost ($){0}".format(calculate_fitness(['cost'], soln)))
...
class TestGASelectionMethods(unittest.TestCase):
    ...
    def test_mutation(self):
        ...
        self.assertSequenceEqual(mutated_order_swapped,
                                 [alloc.task.tid for alloc in
                                  mutated_child.execution_order])
    ...
    def test_overall(self):
        ...
        ax.set_xlim([80, 250])
        ax.set_ylim([100, 250])
        scatter = ax.scatter(generations[0][0], generations[0][1], c='blue',
                             alpha=.2, label='New Pop')
        parent_scatter = ax.scatter(parents1[0][0], parents1[0][1], c='red',
                                    alpha=0.8, label='Parent 1')
        parent2_scatter = ax.scatter(parents2[0][0], parents1[0][1], c='red',
                                     alpha=0.8, label='Parent 2')
        ax.legend()
