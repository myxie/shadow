...
def generate_system_machines(num_machines, magnitude='giga', heterogeneity=[1.0], specrange=[(10, 20)]):
	...
	:return:
	"""
	multiplier = 1000000  # default is Giga flops
	...
	for i, m in enumerate(machines):
		...
		machine_categ['cat{0}'.format(i)] = [{'m{0}'.format(x): rnd - (rnd % multiplier)} for x in range(int(m))]
	...
	if len(args) > 1:
		...
		for path in sorted(os.listdir('test/')):
			if 'dot' in path:
				...
				generate_graph_costs('test/{0}'.format(path), 'test/json/{0}.json'.format(path[:-4]), 1, 100, 50,
									 'giga')
				generate_system_machines(4, 'giga', [0.5, 0.5], [(10, 20), (5, 10)])
